...
BEGIN
    ...
    INSERT INTO topics (user_id, name, color) VALUES (v_uid, 'Демо: Курсовая PostgreSQL', '#2563EB')
    ON CONFLICT (user_id, name) DO UPDATE SET color = EXCLUDED.color
    RETURNING id INTO v_topic;
    v_reg := jsonb_set(v_reg, '{topics}', (v_reg->'topics') || to_jsonb(v_topic));

    INSERT INTO topics (user_id, name, color) VALUES (v_uid, 'Демо: Pet-проект', '#7C3AED')
    ON CONFLICT (user_id, name) DO UPDATE SET color = EXCLUDED.color
    RETURNING id INTO v_topic;
    v_reg := jsonb_set(v_reg, '{topics}', (v_reg->'topics') || to_jsonb(v_topic));

    INSERT INTO topics (user_id, name, color) VALUES (v_uid, 'Демо: Экзамены', '#DC2626')
    ON CONFLICT (user_id, name) DO UPDATE SET color = EXCLUDED.color
    RETURNING id INTO v_topic;
    v_reg := jsonb_set(v_reg, '{topics}', (v_reg->'topics') || to_jsonb(v_topic));

    INSERT INTO topics (user_id, name, color) VALUES (v_uid, 'Демо: Дом и быт', '#059669')
    ON CONFLICT (user_id, name) DO UPDATE SET color = EXCLUDED.color
    RETURNING id INTO v_topic;
    v_reg := jsonb_set(v_reg, '{topics}', (v_reg->'topics') || to_jsonb(v_topic));
    ...
    INSERT INTO tasks (
        ...
        v_uid, v_topics[1 + (13 % array_length(v_topics, 1))], '[демо] Исправить XSS in diary search',
        'Автоматически сгенерированная демо-задача для тестирования UI и статистики.',
        'urgent'::task_priority_enum, 'pending'::task_status_enum,
        NULL, now() - 4 * INTERVAL '1 day' + TIME '18:00',
    ...
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #1: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 0 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #2: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 1 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #3: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 2 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Изучал OLAP-срезы и материализованные представления v_overdue_tasks. Запись #4: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 3 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #5: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 4 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #6: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 5 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #7: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 7 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #8: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 8 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #9: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 9 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #10: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 10 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #11: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 11 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #12: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 12 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #13: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 14 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #14: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 15 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Стресс на работе, но удержался — markers показывают один эпизод тяги. Запись #15: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 16 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #16: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 17 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #17: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 18 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #18: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 19 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #19: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 21 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #20: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 22 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Изучал OLAP-срезы и материализованные представления v_overdue_tasks. Запись #21: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 23 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #22: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 24 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #23: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 25 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Стресс на работе, но удержался — markers показывают один эпизод тяги. Запись #24: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 26 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #25: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 28 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #26: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 29 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #27: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 30 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #28: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 31 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #29: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 32 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Изучал OLAP-срезы и материализованные представления v_overdue_tasks. Запись #30: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 33 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #31: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 35 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Стресс на работе, но удержался — markers показывают один эпизод тяги. Запись #32: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 36 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #33: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 37 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #34: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 38 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #35: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 39 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #36: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 40 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #37: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 42 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Изучал OLAP-срезы и материализованные представления v_overdue_tasks. Запись #38: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 43 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #39: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 44 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #40: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 45 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Стресс на работе, но удержался — markers показывают один эпизод тяги. Запись #41: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 46 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #42: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 47 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #43: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 49 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #44: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 50 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #45: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 51 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #46: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 52 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Изучал OLAP-срезы и материализованные представления v_overdue_tasks. Запись #47: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 53 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #48: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 54 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Стресс на работе, но удержался — markers показывают один эпизод тяги. Запись #49: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 56 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #50: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 57 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #51: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 58 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #52: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 59 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #53: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 60 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #54: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 61 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Изучал OLAP-срезы и материализованные представления v_overdue_tasks. Запись #55: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 63 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #56: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 64 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #57: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 65 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Стресс на работе, но удержался — markers показывают один эпизод тяги. Запись #58: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 66 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #59: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 67 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #60: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 68 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #61: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 70 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #62: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 71 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #63: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 72 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Изучал OLAP-срезы и материализованные представления v_overdue_tasks. Запись #64: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 73 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #65: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 74 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #66: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 75 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #67: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 77 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #68: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 78 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #69: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 79 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сегодня работал над курсовой по PostgreSQL: view, процедуры, триггеры. Запись #70: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 80 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Плотный день: задачи, Pomodoro, паттерны habit и markers. Запись #71: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 81 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Энергии мало, но закрыл несколько демо-задач и записал время. Запись #72: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 82 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Сценарий дня прошёл — отметил триггеры и исход в pattern insights. Запись #73: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 84 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 1; v_energy := 3;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Отличное настроение: streak по зарядке вырос, дневник заполнен. Запись #74: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 85 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 2; v_energy := 4;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Стресс на работе, но удержался — markers показывают один эпизод тяги. Запись #75: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 86 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 3; v_energy := 5;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Планировал неделю: повторяющиеся задачи и цели на месяц. Запись #76: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 87 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 4; v_energy := 1;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Тестировал import/export JSON — merge и restore отработали корректно. Запись #77: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 88 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
    ...
    v_mood := 5; v_energy := 2;
    INSERT INTO diary_entries (user_id, entry_date, content, mood, energy)
    VALUES (v_uid, d, '[демо-дневник] Календарь показывает плотные дни — много task_time_logs и diary. Запись #78: PostgreSQL FTS, mood/energy, теги.',
            v_mood, v_energy)
    ON CONFLICT (user_id, entry_date) DO UPDATE
        SET content = EXCLUDED.content, mood = EXCLUDED.mood, energy = EXCLUDED.energy, updated_at = now()
    RETURNING id INTO v_entry;
    ...
    IF v_tags IS NOT NULL AND array_length(v_tags, 1) >= 1 THEN
        ...
        ON CONFLICT DO NOTHING;
        IF 89 % 3 = 0 AND array_length(v_tags, 1) >= 2 THEN
            INSERT INTO diary_tags (entry_id, tag_id)
