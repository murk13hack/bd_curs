...
В тексте ниже процессы описаны словами; **рисунки 3–8** вставляются по заглушкам `[Вставить рисунок N]` (исходники — `docs/diagrams/`, перечень — [KURSOVAYA_FIGURES.md](./KURSOVAYA_FIGURES.md)).
...
> **[Вставить рисунок 3]** — экспорт `docs/diagrams/03-bpmn-complete-task.bpmn`.  
> *Подпись в Word: «Рисунок 3 — BPMN процесса завершения задачи».*

#### Рисунок 5 — Диаграмма состояний `tasks.status`

> **[Вставить рисунок 5]** — экспорт `docs/diagrams/05-state-task.puml`.  
> *Подпись в Word: «Рисунок 5 — Диаграмма состояний `tasks.status`».*
...
> **[Вставить рисунок 4]** — экспорт `docs/diagrams/04-bpmn-pattern-habit.bpmn`.  
> *Подпись в Word: «Рисунок 4 — BPMN суточного цикла паттерна habit».*

#### Рисунок 6 — Диаграмма состояний `pattern_logs.status`

> **[Вставить рисунок 6]** — экспорт `docs/diagrams/06-state-pattern-log.puml`.  
> *Подпись в Word: «Рисунок 6 — Диаграмма состояний `pattern_logs.status`».*
...
> **[Вставить рисунок 7]** — экспорт `docs/diagrams/07-activity-daily.puml`.  
> *Подпись в Word: «Рисунок 7 — Диаграмма деятельности планировщика БД».*

#### Рисунок 8 — Последовательность запроса календаря месяца

> **[Вставить рисунок 8]** — экспорт `docs/diagrams/08-sequence-calendar.mmd`.  
> *Подпись в Word: «Рисунок 8 — Последовательность запроса календаря месяца».*
...
> **[Вставить рисунок 2]** — экспорт `docs/diagrams/02-architecture.mmd`.  
...
> Исходник рисунка: [`docs/diagrams/01-er-full.mmd`](./diagrams/01-er-full.mmd). В тексте записки — **рисунок 1** (см. [§ 5.2](#52-er-диаграмма-полная-по-реализации)).
...
> **[Вставить рисунок 1]** — экспорт PNG/SVG из `docs/diagrams/01-er-full.mmd` (Mermaid Live / draw.io).  
...
**Фактический набор S2** (прогон `./scripts/benchmark_run_for_kursovaya.sh 10000`, см. `docs/benchmark_explain_out.txt`): **10 060** задач (`10 000` bench + demo), **578** записей дневника, **11** паттернов, **149** строк `pattern_logs`.
...
> **[Вставить рисунок 11]** — фрагмент `docs/benchmark_explain_out.txt`, блок Q1 (или [приложение Е](./KURSOVAYA_APPENDIX.md)).  
> *Подпись в Word: «Рисунок 11 — План запроса календаря месяца при наборе S2».*

#### Рисунок 12 — Планы FTS дневника (Q2a / Q2b)

> **[Вставить рисунок 12]** — фрагменты `docs/benchmark_explain_out.txt`, блоки Q2a и Q2b.  
> *Подпись в Word: «Рисунок 12 — Планы `fn_search_diary` до и после индекса GIN».*

#### Рисунок 13 — План выборки задач по индексу

> **[Вставить рисунок 13]** — фрагмент `docs/benchmark_explain_out.txt`, блок Q3.  
